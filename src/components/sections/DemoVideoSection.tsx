...
const CURSOR_POS = [
  { left: '55%', top: '40%' },  // 0: idle
  { left: '9%',  top: '6%'  },  // 1: to navbar button (top-left)
  { left: '9%',  top: '6%'  },  // 2: click
  { left: '50%', top: '44%' },  // 3: popup
  { left: '50%', top: '56%' },  // 4: form
  { left: '50%', top: '76%' },  // 5: submit
  { left: '55%', top: '50%' },  // 6: neutral
  { left: '55%', top: '50%' },  // 7: whatsapp visible
  { left: '55%', top: '40%' },  // 8: reset
];
      ...
      <div className="max-w-3xl mx-auto" dir="rtl">
        ...
        >
          ...
          <div className="relative overflow-hidden" style={{ height: 390 }}>
            ...
            {/* Mini Navbar — mirrors real site */}
            <div
              className="absolute top-0 left-0 right-0 flex items-center justify-between px-4 select-none"
              dir="rtl"
              style={{
                height: 42,
                background: 'rgba(14,18,26,0.85)',
                backdropFilter: 'blur(12px)',
                borderBottom: '1px solid hsl(215,20%,14%)',
                zIndex: 10,
              }}
            >
              {/* Logo */}
              ...
              {/* CTA button — matches real Navbar exactly */}
              <div
                className="relative transition-all duration-500"
                style={{
                  transform: buttonClick ? 'scale(0.93)' : buttonGlow ? 'scale(1.06) translateY(-1px)' : 'scale(1)',
                }}
              >
                {/* Glow layer */}
                <div
                  className="absolute inset-0 rounded-full blur-[10px] transition-opacity duration-300"
                  style={{
                    background: 'hsl(160,84%,39%)',
                    opacity: buttonGlow ? 0.45 : 0,
                  }}
                />
                <div
                  className="relative flex items-center gap-1.5 px-3 py-1.5 rounded-full text-[11px] font-medium cursor-default"
                  style={{ background: 'hsl(160,84%,39%)', color: '#fff' }}
                >
                  שיחת אסטרטגיה
                  <span
                    className="w-1.5 h-1.5 rounded-full flex-shrink-0"
                    style={{ background: 'rgba(255,255,255,0.8)', animation: 'blink 1.2s ease-in-out infinite' }}
                  />
                </div>
              </div>
            </div>
            ...
            {/* Bot icon + proactive bubble — mirrors real site (bottom-right) */}
            <div
              className="absolute bottom-4 right-4 flex flex-col items-end gap-2 select-none"
              style={{ opacity: showPopup ? 0.2 : 1, transition: 'opacity 0.5s', zIndex: 5 }}
            >
              <div
                className="rounded-2xl rounded-br-none px-3 py-2 text-[10px] leading-relaxed"
                style={{
              ...
              >
                היי 👋 רוצה לראות איזה אוטומציות יכולות לעבוד בעסק שלך?
              </div>
              <img
                src="/bot-icon.png"
                alt="bot"
