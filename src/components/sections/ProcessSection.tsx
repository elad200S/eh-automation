...
import { cardReveal, staggerContainer, fadeUp } from '@/lib/animations';
    ...
    {['מיפוי תהליכים קיימים', 'איתור נקודות כשל', 'הגדרת יעדים'].map((item, i) => (
      ...
      >
        <div
          ...
          style={{ background: 'hsl(210 100% 58% / 0.15)', animation: `checkPop 3s ease-in-out ${i * 0.6}s infinite` }}
        >
...
const FlowAnimation = () => (
  <div className="mt-5 flex items-center justify-center gap-1">
    ...
    <div className="relative w-8 h-1 flex items-center">
      ...
      <div className="absolute w-2 h-2 rounded-full bg-primary" style={{ animation: 'travelDot 2s ease-in-out infinite' }} />
    </div>
    ...
    <div className="relative w-8 h-1 flex items-center">
      <div className="w-full h-px bg-border" />
      <div className="absolute w-2 h-2 rounded-full bg-secondary" style={{ animation: 'travelDot 2s ease-in-out 0.6s infinite' }} />
    </div>
  ...
  <div className="mt-5 flex items-end justify-center gap-3 h-12">
    {[
      ...
      <div
        ...
        style={{ height: bar.height, background: bar.color, opacity: 0.8, animation: `barGrow 2.5s ease-in-out ${bar.delay} infinite`, transformOrigin: 'bottom' }}
      />
    ))}
  </div>
);
...
const steps = [
  { title: 'הבנה של העסק והתהליך הקיים', description: 'איפה הולכים לאיבוד לידים ואיפה יש בזבוז זמן', Icon: Search, number: '01', Animation: ChecklistAnimation },
  { title: 'בניית אוטומציה מותאמת', description: 'חיבור בין המערכות והגדרת זרימות עבודה', Icon: Settings, number: '02', Animation: FlowAnimation },
  { title: 'הטמעה ושיפור', description: 'בדיקות, התאמות ושיפור מתמשך לפי הצורך', Icon: Rocket, number: '03', Animation: BarChartAnimation },
];
...
const ProcessSection = () => {
  return (
    <Section id="process">
      <style>{`
        @keyframes fadeSlide { 0%,10%{opacity:0;transform:translateX(6px)} 25%,70%{opacity:1;transform:translateX(0)} 85%,100%{opacity:0;transform:translateX(0)} }
        @keyframes checkPop  { 0%,10%{transform:scale(0.6)} 25%,70%{transform:scale(1)} 100%{transform:scale(1)} }
        @keyframes travelDot { 0%{left:0;opacity:0} 10%{opacity:1} 90%{opacity:1} 100%{left:calc(100% - 8px);opacity:0} }
        @keyframes barGrow   { 0%,10%{transform:scaleY(0)} 40%,70%{transform:scaleY(1)} 90%,100%{transform:scaleY(0.2)} }
        @media (prefers-reduced-motion: reduce) { .process-anim *{animation:none!important} }
      `}</style>
      ...
      <div className="max-w-4xl mx-auto">
        ...
        >
          {steps.map((step, index) => {
            ...
            return (
              <motion.div key={index} variants={cardReveal} className="relative">
